...
{
    // Build the 8-page source to a temp file so PdfDocumentReader can open it.
    $tmp = tempnam(sys_get_temp_dir(), 'phppdf_nup_');
    buildSourceDocument($tmp);

    $srcDoc = PdfDocumentReader::open($tmp);

    // 2-up: A4 landscape (842 × 595), 2 columns × 1 row → 4 output sheets.
    $config = NUpConfig::twoUp(842, 595);
    $result = (new NUpImposer($srcDoc, $config))->impose(
        (new PdfDocumentInfo())->title('2-up Imposition')->author('phppdf'),
    );
